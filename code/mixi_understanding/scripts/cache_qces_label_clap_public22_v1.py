#!/usr/bin/env python3
"""Cache exact frozen AudioSep-CLAP text embeddings for the public 22 labels."""
...
FORMAT = "qces_label_clap_public22_v1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--ontology", type=Path,
        default=PROJECT_ROOT / "code/mixi_understanding/resources/qces_public22_ontology.txt",
    )
    parser.add_argument(
    ...
    parser.add_argument("--seed", type=int, default=2241)
...
def main() -> None:
    ...
    if len(labels) != 22 or len(labels) != len(set(labels)):
        raise ValueError(f"requires the frozen public 22-label ontology, got {len(labels)}")
    ...
    if embeddings.shape != (22, 512) or not bool(torch.isfinite(embeddings).all()):
    ...
    try:
        ...
        if check.get("format") != FORMAT or tuple(check["embeddings"].shape) != (22, 512):
