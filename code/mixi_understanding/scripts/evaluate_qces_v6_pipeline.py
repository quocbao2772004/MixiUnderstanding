#!/usr/bin/env python3
"""Evaluate QCES-v6 evidence separation and every comparison baseline.

One pass over a manifest produces, for each requested mode, an evidence stem and
its residual, and scores them with the frozen QCES-v5 waveform metrics.  The
modes differ only in where the text prompt and the temporal gate come from, so
the table isolates exactly which oracle each system still needs.
...
FORMAT_VERSION = "qces_v6_pipeline_evaluation_v1"
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    proposal_cache: dict[tuple[str, str, tuple[str, ...]], list[EventProposal]] = {}

    def proposals_for(
        source: str, scene_id: str, labels: tuple[str, ...]
    ) -> list[EventProposal]:
        key = (source, scene_id, labels)
        ...
        for mode in args.modes:
            if mode == "mixture_passthrough":
                ...
                if mode == "oracle_inventory__planned_gate":
                    ...
                    inventory = proposals_for("learned", record.scene_id, labels)
                plan = plan_from_proposals(parsed, inventory)
                if mode == "predicted__best_effort":
    ...
    prompts = sorted(
        {
            ...
            if request.prompt and not request.silent
        }
    )
    embeddings = (
        encode_prompts(
    ...
    with torch.inference_mode():
        for record in records:
            ...
            for mode in args.modes:
                ...
                elif request.silent or not request.prompt:
